...
{
    queue <int> Q;
    FOR(i,1,n)  // first, i am here inserting independent node in queue
    {
        if(inDegree[i] == 0)
            Q.push(i);
    }
...
int main()
{
    ...
    FOR(t,1,T)
    {
        ...
        FOR(i,1,n)
        {
            string x,y;
            cin>>x>>y;
            graph[mapFind(x)].pb(mapFind(y));
            myset.insert(mapFind(x)); // inserting for counting total unique input
            myset.insert(mapFind(y));
            inDegree[mapFind(y)]++;

        }
        int sz = tophSort(myset.size()); // invoking by total size unique input
        pcase(t);
        if(sz==myset.size())
            cout<<"Yes"<<endl;
        else
