...
int n,t,flag;
vi v;
int knapsack(int curr, int space)
{
    if(curr == t)
        return space;

    int p = -123;
    if(space+track[curr] <= n)
        p = knapsack(curr+1,space+track[curr]);

    int q = knapsack(curr+1,space);
    return  MAX(p,q);
}
